...
function update_header(colourr) {
	...
	function isNight(color) {
		...
		if (hsp > 127) { // hsp>127.5 Using the HSP value, determine whether the color is light or dark
			return false; //'day'
		}
		else {
			return true; //'night'
	...
	for (var i = 0; i < sections_color.length; i++) {
		...
		if ((window.scrollY >= offset1) && (window.scrollY < offset2)) {
			...
			if (isNight(new_colourr)) {
				...
				$('.menu_button').addClass('night'); $('.nav-bar-container .nav-link-expandable').addClass('night'); $('.nav-bar-container > .nav-link-expandable').addClass('night');
				...
				$('.menu_button').removeClass('night'); $('.nav-bar-container .nav-link-expandable').removeClass('night'); $('.nav-bar-container > .nav-link-expandable').removeClass('night');
